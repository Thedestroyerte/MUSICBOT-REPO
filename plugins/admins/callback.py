...
from Zero.config import (AUTO_DOWNLOADS_CLEAR, BANNED_USERS,
...
from Zero.misc import SUDOERS, db
from Zero.utils.database import (is_active_chat,
...
from Zero.utils.formatters import seconds_to_min
from Zero.utils.inline.play import (stream_markup,
    stream_markup_timer,
    telegram_markup,
    telegram_markup_timer,
    close_keyboard)
from Zero.utils.stream.autoclear import auto_clean
from Zero.utils.thumbnails import gen_thumb
...
async def del_back_playlist(client, CallbackQuery, _):
    ...
    if command == "Pause":
        ...
        await CallbackQuery.answer()
        await Zeroo.stop_stream(chat_id)
        ...
        if "live_" in queued:
            ...
            try:
                await Zeroo.skip_stream(chat_id, link, video=status)
            except Exception:
                return await CallbackQuery.message.reply_text(
                    _["call_9"]
                )
            ...
            try:
                await Zeroo.skip_stream(chat_id, queued, video=status)
            except Exception:
                return await CallbackQuery.message.reply_text(
                    _["call_9"]
                )
